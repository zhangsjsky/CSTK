...
class KGMLCanvas(object):
    ...
    def draw(self, filename):
        ...
        # Instantiate canvas
        self.drawing = \
            canvas.Canvas(filename+'.pdf', bottomup=0,
                          pagesize=(cwidth *
                                        (1 + 2 * self.margins[0]),
                                    cheight *
                                        (1 + 2 * self.margins[1])))
        ...
        if self.import_imagemap:
            ...
            self.drawing.drawImage(imfilename, 0, 0,mask = [1,2,1,2,1,2])
 ...
 #               self.__draw_arrow(subtype, relation.entry2)
            self.__draw_arrow(relation.entry1, relation.entry2)
