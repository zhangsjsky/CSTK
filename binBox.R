...
Option:
    ...
    -x|xlab     STR     The xlab[Binned Value]
    -y|ylab     STR     The ylab
    ...
    -b|break    STR     The break number or break points in vector[10]
    -h|help             Show help
    
    R base specific:
    -no|noOutliner      Don't draw outlier

    ggplot specific:
    
    -l|length       DOU     The bin width
...
if(length(args) >= 1){
    for(i in 1:length(args)){
        ...
        tmp = parseArg(arg, 'b(reak)?', 'b'); if(!is.null(tmp)) myBreak = tmp
        if(arg == '-no' || arg == '-noOutliner') outlier = FALSE
        tmp = parseArgAsNum(arg, 'l(ength)?', 'l'); if(!is.null(tmp)) length = tmp
...
if(exists('noGgplot')){
  myCmd = paste0('myBinFactor = cut(data[[1]], dig.lab = 5, breaks = ', myBreak, ')')
  
  myCmd = paste0(myCmd, '; boxplot(data[[2]]~myBinFactor, outline = outlier')
  if(exists('yLog')) myCmd = paste0(myCmd, ', log = "y"')
  ...
  if(exists('myHorizontal')) abline(h = myHorizontal, lty = 2)
}else{
    library(ggplot2)
    options(scipen = 20)
    myCmd = paste0('p = ggplot(data, aes(cut(V1, dig.lab = 5, breaks = ', myBreak)
    myCmd = paste0(myCmd, '), V2))')
    eval(parse(text = myCmd))
    ...
    if(!exists('noJitter')) p = p + geom_jitter(alpha = 0.5)
