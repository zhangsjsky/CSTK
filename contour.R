...
Option:
    ...
    ggplot specific:
    -b|bin      DOU     The bin number
    -binWidth   DOU     The bin width
    -geom       STR     The geometric object to use display the data ([path], polygon)
...
if(length(args) >= 1){
    for(i in 1:length(args)){
        ...
        tmp = parseArgAsNum(arg, 'binWidth', 'binWidth'); if(!is.null(tmp)) binWidth = tmp
...
myCmd = 'p = p + stat_contour(show_guide = showGuide, '
