...
Option:
    ...
    -k|kernel       STR Kernel used for density estimation ([gaussian], rectangular, 
                        triangular, epanechnikov, biweight, cosine, optcosine)
    -adjust         DOU Adjust the bandwidth[1]
    -trim               Trim densities to the actual range of the data, 
                        otherwise they are extended by the default 3 bandwidths

    -a|alpha    DOU     The alpha of density body
    ...
    -c|color    STR     The color of density boundary
    ...
    -f|fill     STR     The color of density body[NA]
    ...
    -s|size     DOU     The size of density boundary
    ...
    -weight     DOU     The weight of density
...
if(exists('noGgplot')){
    ...
    myCmd = paste0('p = p + geom_density(show_guide = showGuide, ')
