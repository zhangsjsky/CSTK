...
Option:
    ...
    ggplot specific:
    -q|quantile STRs    Comma-separated quantiles[0.25, 0.5, 0.75]
    -point              Draw points
...
if(exists('noGgplot')){
    ...
    myCmd = paste0('p = p + geom_quantile(show_guide = showGuide')
    if(exists('quantiles')) myCmd = paste0(myCmd, ', quantiles = quantiles')
    ...
    if(exists('point')) p = p + geom_point()
