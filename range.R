...
Option:
    ...
    -r|rangeType    STR     The range type([pointrange], linerange, errorbar, crossbar)
    -barWidth       DOU     The width of bar[0.9]
    -dL|drawLine            Draw a line across the y

    -a|alpha        DOU     The alpha of range body
    -alphaV         STR     The column name to apply alpha (V5, V6, ...)
    ...
    -c|color        STR     The color of range boundary
    -colorV         STR     The column name to apply color (V5, V6,...)
    ...
    -f|fill         STR     The color of range body
    -fillV          STR     The column name to apply fill (V5, V6,...)
    ...
    -linetypeV      STR     The column name to apply linetype (V5, V6,...)
    ...
    -s|size         DOU     The size of range body
    -sizeV          STR     The column name to apply size (V5, V6,...)
    ...
    -weight         DOU     The weight of range
    -weightV        STR     The column name to apply weight (V5, V6,...)
    ...
    -fp|flip                Flip the plot to horizontal
...
if(length(args) >= 1){
    for(i in 1:length(args)){
        ...
        tmp = parseArg(arg, 'r(angeType)?', 'r'); if(!is.null(tmp)) rangeType = tmp
...
if(exists('noGgplot')){
    ...
    p = ggplot(data, aes(x = factor(V1, levels = V1), y = V2, ymin = V3, ymax = V4))
    ...
    myCmd = paste0('p = p + geom_', rangeType, '(show_guide = showGuide')
    ...
    if(exists('drawLine')) p = p + geom_line(aes(x = 1:nrow(data), V2))
    p
}