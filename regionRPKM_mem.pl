...
print <<HELP;
Usage: perl $scriptName -b region.bed INPUT.bam >rpkm.bed
...
Note: INPUT.bam should be indexed with samtools index
      This script is for handling big bam file (e.g.: merged bam) that can't be entirely cached into memory.
      It's low MEMORY-CONSUMED but TIME-CONSUMED compared to its equivalent regionRPKM_speed.pl
...
if(-f $ARGV[0]){
    ...
    die "Please offer the reads file\n";
}
