...
Option:
    ...
    ggplot specific:
    -ymax       DOU     Set the constant Y max
    -ymin       DOU     Set the constant X max

    -a|alpha    DOU     The alpha of ribbon body
    ...
    -c|color    STR     The color of ribbon boundary
    ...
    -f|fill     STR     The color of ribbon body
    ...
    -s|size     DOU     The size of ribbon body
...
if(length(args) >= 1){
    for(i in 1:length(args)){
        ...
        tmp = parseArgAsNum(arg, 'ymax', 'ymax'); if(!is.null(tmp)) ymax = tmp
        tmp = parseArgAsNum(arg, 'ymin', 'ymin'); if(!is.null(tmp)) ymin = tmp
...
myCmd = 'p = p + geom_ribbon(show_guide = showGuide, aes(ymin = '
if(exists('ymin'))
  myCmd = paste0(myCmd, ymin)
}else{
  myCmd = paste0(myCmd, 'V2')
}
myCmd = paste0(myCmd, ', ymax = ')
if(exists('ymax')){
  myCmd = paste0(myCmd, ymax)
}else{
 myCmd = paste0(myCmd, 'V3')
}
myCmd = paste0(myCmd, ')')
...
eval(parse(text = myCmd))
if(ncol(data) >= 4 ) p = p + geom_line(aes(y = V4))
