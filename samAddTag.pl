...
while(<IN>){
    ...
    if(defined $coverage){
        ...
        $coverage = sprintf "%.2f", $alignedLength / $readLength * 100;
    ...
    if(defined $identity){
        ...
        my $identity = sprintf "%.2f", $matchLength / $alignedLength * 100;
    ...
    if(defined $list){
        push @addedTags, "$listTag:$readName2TagValue{$readName}" if exists $readName2TagValue{$readName};
    }
