...
Option:
    ...
    -fY         STR     The fun.y parameter([mean], median, sd, min, max)
    -g|geom     STR     The The geometric object to use display the summary ([pointrange], smooth, errorbar, line, crossbar)
    -fI         STR     The fun.min parameter(min, ...)
    -fA         STR     The fun.max parameter(max, ...)
...
if(length(args) >= 1){
    for(i in 1:length(args)){
        ...
        tmp = parseArg(arg, 'fY', 'fY'); if(!is.null(tmp)) fY = tmp
        tmp = parseArg(arg, 'g(eom)?', 'g'); if(!is.null(tmp)) geom = tmp
        tmp = parseArg(arg, 'fI', 'fI'); if(!is.null(tmp)) fI = tmp
        tmp = parseArg(arg, 'fA', 'fA'); if(!is.null(tmp)) fA = tmp
...
if(exists('noGgplot')){
    ...
    myCmd = paste0('p = p + stat_summary(show.legend = showGuide, fun.y = fY')
    if(exists('geom')) myCmd = paste0(myCmd, ', geom = geom')
    if(exists('fI')) myCmd = paste0(myCmd, ', fun.min = fI')
    if(exists('fA')) myCmd = paste0(myCmd, ', fun.max = fA')
