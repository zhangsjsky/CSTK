...
Option:
    ...
    -b|bin      DOU     The bin number on X and Y[30]
    -fSum       STR     The summary funtion (mean, median, var, ...)
...
if(length(args) >= 1){
    for(i in 1:length(args)){
        ...
        tmp = parseArg(arg, 'f(ill)?', 'fill'); if(!is.null(tmp)) fill = tmp
...
if(exists('noGgplot')){
    ...
    myCmd = paste0('p = p + stat_summary_hex(na.rm = TRUE, bins = bin')
    if(exists('fSum')) myCmd = paste0(myCmd, ', fSum = fSum')
